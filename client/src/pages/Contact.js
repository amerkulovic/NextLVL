...
const Contact = () => {
  ...
  const [nameStyling, setNameStyling] = useState("w-1/2 max-sm:w-10/12 h-10");
  const [emailInput, setEmailInput] = useState("");
  const [emailStyling, setEmailStyling] = useState("w-1/2 max-sm:w-10/12 h-10");

  const nameInputHandler = (event) => {
    if (nameInput.trim().length > 0) {
      setNameStyling("w-1/2 max-sm:w-10/12 h-10");
    }
  ...
  const emailInputHandler = (event) => {
    if (emailInput.trim().length > 0) {
      setEmailStyling("w-1/2 max-sm:w-10/12 h-10");
    }
  ...
  const submitHandler = (event) => {
    ...
    if (nameInput === "") {
      setNameStyling("w-1/2 max-sm:w-10/12 border border-red-600 bg-red-200 h-10");
    } else {
      setNameStyling("w-1/2 max-sm:w-10/12 h-10");
    }
    if (emailInput === "") {
      setEmailStyling("w-1/2 max-sm:w-10/12 border border-red-600 bg-red-200 h-10");
    } else {
      setEmailStyling("w-1/2 max-sm:w-10/12 h-10");
    }
  };
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-[#2C2E43] from-7% via-white via-30% to-[#2C2E43] to-98%  py-10">
        <div className="flex flex-row max-md:flex-col justify-center">
          <div className="bg-yellow-400 flex flex-col px-10 border-4 border-y-black border-l-black border-r-yellow-400 max-md:border-r-black max-md:mb-3 max-md:text-center">
            ...
            <ul className="pt-20 font-light max-md:pb-8">
          ...
          <form className="bg-yellow-400 text-center pt-20 pb-10 w-1/2 max-sm:w-full max-lg:w-3/4 border-4 border-y-black border-r-black border-l-yellow-400 max-md:border-l-black max-md:w-full">
            ...
            <div className="py-3 flex flex-col justify-center items-center">
              ...
              <textarea className="w-1/2 max-sm:w-10/12" rows="5" name="message"></textarea>
            </div>
